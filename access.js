...
(() => {
  ...
  function remember(value){token=value;try{if(value)sessionStorage.setItem(sessionKey,value);else sessionStorage.removeItem(sessionKey)}catch{}}
  ...
  async function loginButton(){
    ...
    document.getElementById('password-form').onsubmit=async event=>{
      event.preventDefault();const form=event.currentTarget,data=new FormData(form),body=Object.fromEntries(data);
      ...
      try{const result=await api(registerMode?'/auth/register':'/auth/login',{method:'POST',body:JSON.stringify(body)});if(generation!==epoch)return;form.reset();remember(result.token);user=null;revision=-1;await refresh()}
